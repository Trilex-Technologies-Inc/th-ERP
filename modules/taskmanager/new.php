...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif (strlen($taskTitle) > 255 || !in_array($priority, $priorities, true))
        $error = 'The task details are invalid.';
    elseif ($dueDate !== '' && !preg_match('/^\d{4}-\d{2}-\d{2}$/', $dueDate))
        $error = 'Enter a valid due date.';
    else {
        sql("INSERT INTO module_taskmanager_task (username, title, description, priority, due_date, completed, created_at) VALUES (" .
            sql_string(getUser()) . ', ' . sql_string($taskTitle) . ', ' . sql_string($description) . ', ' . sql_string($priority) . ', ' . ($dueDate === '' ? 'NULL' : sql_string($dueDate)) . ", 0, NOW())");
        taskManagerRedirect('Task created.');
    }
}
taskManagerPageStart('New task');
?>
<section class="task-form-card card border-0 shadow-sm">
    <div class="card-header bg-white"><span class="module-eyebrow">Task Manager</span><h1>New task</h1></div>
    <div class="card-body">
        <?php if ($error !== '') { ?><div class="alert alert-danger"><?php echo htmlspecialchars($error) ?></div><?php } ?>
        <form method="post" action="<?php echo htmlspecialchars(moduleUrl('taskmanager', 'new')) ?>">
            ...
            <div class="d-flex gap-2"><button class="btn btn-primary" type="submit">Create task</button><a class="btn btn-outline-secondary" href="<?php echo htmlspecialchars(moduleUrl('taskmanager')) ?>">Cancel</a></div>
        </form>
    </div>
</section>
